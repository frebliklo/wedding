...
const Wishlist = ({ location }) => (
  <Layout marginTop={48} footer={true} path={location.pathname}>
    <SEO
      title="Ønskeliste"
      keywords={['bryllup', 'ønskeliste', 'praktisk info']}
    />
    <section css={styles.section}>
      <Container>
        <h1 css={styles.title}>Ønskeliste</h1>
        <p>
          Vi ønsker os <em css={{ margin: '0 .25rem 0 .125rem' }}>allermest</em>{' '}
          at komme på en skøn bryllupsrejse.
        </p>
        <p css={{ marginBottom: '.75rem' }}>
          Derfor kan bidrag til denne overføres til:
        </p>
        <div css={styles.accountContainer}>
          <div css={styles.accountLine}>
            <p css={{ ...styles.accountColumnOne, ...styles.accountText }}>
              Reg:
            </p>
        ...
        <ul>
          {list.map(item => (
            <ListItem key={item.key} text={item.text} link={item.link} />
          ))}
        </ul>
      </Container>
    </section>
